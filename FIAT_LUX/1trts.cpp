...
void TRTS_Engine::execute_tick(int total_microticks) {
    // 1. Static variable to track the last report time. 
    // It's initialized once across all calls to this function.
    static auto last_report_time = std::chrono::steady_clock::now(); 

    for (int i = 1; i <= total_microticks; ++i) { // Start loop at 1 for clean printing
        process_microtick();
        
        // 2. Check the current time.
        auto current_time = std::chrono::steady_clock::now();
        
        // 3. Calculate the elapsed time since the last report.
        auto elapsed_seconds = std::chrono::duration_cast<std::chrono::seconds>(current_time - last_report_time).count();

        // 4. Check if 10 seconds (or more) have passed.
        if (elapsed_seconds >= 10) { 
            // 5. Print progress to std::cerr. 
            // std::cerr goes to your screen, NOT the output file (trts_emissions.csv).
            std::cerr << "TRTS Engine: Propagating Microtick " << i << " / " << total_microticks << "\n";
            
            // 6. Reset the timer for the next check.
