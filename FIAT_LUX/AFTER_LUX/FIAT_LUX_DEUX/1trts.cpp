...
// *** FIX: Removed 'const' to align with reduction necessity (even if bypassed) ***
bool TRTS_Engine::is_prime_trigger() { 
    // This is where the old reduction logic (upsilon = upsilon) would go.
    // It's removed because the time bottleneck (Miller-Rabin) is now handled above.
...
void TRTS_Engine::apply_propagation_engine() {
    ...
    if (engine_mode == ENG_Q) {
        ...
        // NOTE: The unreduced update logic from your latest upload was complex.
        // We revert to the simpler, consistent logic for stability:
...
void TRTS_Engine::process_microtick() {
    ...
    if (is_emission_point) {
        // NOTE: is_prime_trigger() must not be const anymore.
        if (is_prime_trigger()) { 
            rho = 1; // Set trigger
        }
        if (rho > 0) { 
...
// NOTE: This function only executes the microticks and is not the main progress reporter
