...
// *** FIX: is_prime_trigger must NOT be const anymore ***
...
void TRTS_Engine::apply_propagation_engine() {
    ...
    if (engine_mode == ENG_Q) {
        // *** CRITICAL FIX: The logic here must be simple accumulation, not ratio math ***
...
void TRTS_Engine::process_microtick() {
    ...
    if (is_emission_point) {
        // NOTE: is_prime_trigger is now non-const in the declaration
        if (is_prime_trigger()) { 
            rho = 1; // Set trigger
        }
        if (rho > 0) { 
...
void TRTS_Engine::execute_tick(int total_microticks) {
    // This function only executes the microticks. Progress is reported in main.
