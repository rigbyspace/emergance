...
PsiBehavior string_to_psi(const std::string& s) {
    ...
    if (s == "CRITICAL") return PSI_CRITICAL_IMBALANCE;
...
TRTS_Engine::TRTS_Engine(PsiBehavior psi, KoppaMode koppa_mode_in, EngineType engine)
    : rho(0), microtick(0), step(0),
      psi_mode(psi),
      koppa_mode(koppa_mode_in),
      engine_mode(engine)
{
...
void TRTS_Engine::process_microtick() {
    ...
    if (is_transformation_point) {
        ...
        if (psi_mode == PSI_CRITICAL_IMBALANCE && koppa.numerator() != koppa.denominator()) {
